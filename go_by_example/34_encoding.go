...
func demoBase64(message string) {
	...
	fmt.Println("base64 msg           :", encoding)

	decoding, _ := base64.StdEncoding.DecodeString(encoding)
	fmt.Println("decoding base64 msg  :", string(decoding))
}
...
func demoHex(message string) {
	...
	fmt.Println("Hex msg              :", encoding)

	decoding, _ := hex.DecodeString(encoding)
	fmt.Println("decoding Hex msg     :", string(decoding))
}
...
func demoXml() {
	...
	emp := &Employee{
		...
		Country: EmployeeCountry{CountryCode: "DE", CountryName: "Denmark"},
	}
...
func demoCsv() {
	...
	file, err := os.Open("../temp/encoding.csv")
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	csvFile, err := os.Create("../temp/employee.csv")
	if err != nil {
